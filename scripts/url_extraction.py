...
def separate_email(text, filename) -> list:
    text = re.sub(r'(\n)|(\s{2,})', ' ', text)
    ...
    if (filename == "spam.csv"):  # dont look for link_regex2 in spam.csv file
        return [text, links, email_addresses]
    links2 = re.findall(link_regex2, text)
    links = links + links2
    content = re.sub(link_regex2, "URL_HERE", text)
    return [content, links, email_addresses]
